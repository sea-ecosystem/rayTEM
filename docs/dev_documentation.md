...
modules; anything beyond those is a regression. Build artifacts
(`docs/_build/`) are not committed. There are no notebooks to execute; the
example scripts run headless via `MPLBACKEND=Agg` and are exercised by the
test suite.
