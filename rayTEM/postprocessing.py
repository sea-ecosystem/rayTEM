...
def plotRays(r1,axis="x"):
	...
	planes=findPlanes(r1) ; ct=0
	for ls,imdiff in zip([":","--"],["diff","image"]):
		...
		for m,i in zip(M,Z):
			...
			z0=zs[i] ; z1=zs[i+1]
			z=z0+(z1-z0)*di
			Xs.append([z,z]) ; Ys.append([-1,1]) ; linestyles.append(ls) ; colors.append("k")
			plt.annotate("M="+str(m),(z,ct))
	
	for x,y,l,c in zip(Xs,Ys,linestyles,colors):
		plt.plot(x,y,linestyle=l,color=c,marker='')
	plt.show()
	#plt.ylim([-5,5])
	#plt.savefig("../../replicateB.svg")
		
# returns nested dicts with keys: ["x" or "y"]["image" or "diff"]["M" or "z"]
# where the value stored is a list of magnitudes or positions (in fractional coordinates!) of each image or diffraction plane in that axis
# "fractional coordinates" meaning: relative to the beginning and end of the given Element. 
# e.g. a z of 1.2 would be 20% of the way through element index 1
# to convert to real-space units, you need to know the position and length of that element
# magnification in real space is defined as: the ratio of final/original positions of the ray
# magnification in diffraction space is defined as: the ratio of final position vs starting angle of the ray
