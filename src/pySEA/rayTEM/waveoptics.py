...
# --- Krivanek/Nion axial aberrations, orders 1 through 5 ---------------------
#
# Keyed by name; the value is (n, m) with `n` the aberration ORDER -- the phase
# term goes as theta**(n+1), the transverse ray aberration as theta**n -- and
# `m` the azimuthal multiplicity, so the term varies as cos(m*phi) and has `m`
# -fold symmetry. `m = 0` terms are rotationally symmetric and carry no
# orientation angle.
#
# order  round (m=0)      the rest
#   1    C1 defocus       A1 twofold astigmatism
#   2    --               B2 axial coma, A2 threefold astigmatism
#   3    C3 spherical     S3 star, A3 fourfold astigmatism
#   4    --               B4 coma, D4 three-lobe, A4 fivefold astigmatism
#   5    C5 spherical     S5 star, R5 rosette, A5 sixfold astigmatism
KRIVANEK_TERMS = {
	'C1': (1, 0), 'A1': (1, 2),
	'B2': (2, 1), 'A2': (2, 3),
	'C3': (3, 0), 'S3': (3, 2), 'A3': (3, 4),
	'B4': (4, 1), 'D4': (4, 3), 'A4': (4, 5),
	'C5': (5, 0), 'S5': (5, 2), 'R5': (5, 4), 'A5': (5, 6),
}


def aberration_phase(shape: tuple, dx: float, dy: float, wavelength: float,
					 coefficients: dict, power: float) -> np.ndarray:
	r"""Axial wave aberration function in Krivanek notation, orders 1 to 5.

	Builds the phase an aberrated round lens imprints, written on the **lens
	plane** where the ray height is proportional to the pupil angle,
	:math:`\theta = r/f`:

	.. math::

		\chi = -k \sum_{n,m} \frac{\theta^{\,n+1}}{n+1}\,
		       C_{n,m}\cos\!\big(m(\phi - \phi_{n,m})\big)

	with :math:`k = 2\pi/\lambda`. Each term is named by the convention a
	corrector is actually tuned in (:data:`KRIVANEK_TERMS`): ``C1`` defocus,
	``A1`` twofold astigmatism, ``B2`` axial coma, ``A2`` threefold, ``C3``
	spherical, ``S3`` star, ``A3`` fourfold, and on through ``C5``/``A5`` at
	fifth order.

	The **sign** follows this module's existing convention, in which a
	converging lens carries :math:`\chi = -k r^2/2f` and the field is multiplied
	by :math:`e^{i\chi}`; that is the opposite sign to texts writing
	:math:`e^{-i\chi}`. With it, a positive ``C3`` focuses marginal rays short,
	as spherical aberration does.
	...
	wavelength : float
		Wavelength (metres).
	coefficients : dict
		``{name: value}`` or ``{name: (value, angle)}``. Names must be keys of
		:data:`KRIVANEK_TERMS`. Magnitudes are in **metres**; the optional angle
		is the term's orientation :math:`\phi_{n,m}` in radians and is rejected
		for the rotationally symmetric ``m = 0`` terms. Zero-valued entries are
		skipped, so an unaberrated lens costs nothing.
	power : float
		Focal power ``1/f`` (1/metres), which converts ray height to pupil
		angle. Must be nonzero for the angle to be defined.

	Returns
	-------
	np.ndarray
		Real phase array χ (radians), shape ``(ny, nx)``. All zeros when no
		nonzero coefficient is given.

	Raises
	------
	KeyError
		If a name is not a Krivanek term this function knows.
	ValueError
		If ``power`` is zero, or an orientation angle is given for an ``m = 0``
		term (which has no orientation), or a coefficient is not a scalar or a
		``(value, angle)`` pair.

	Related
	-------
	KRIVANEK_TERMS : The (order, multiplicity) table and its naming.
	spherical_phase : The ``C3``-only special case, kept for its own tests.
	elements.Lens.phase_shift : Applies this as a residual screen on U.

	Notes
	-----
	This is the **axial** aberration function: coefficients describe the pencil
	on axis, and no field dependence is included, so it is a function of pupil
	position alone. The azimuth is measured in the lens plane's own coordinates;
	for odd ``m`` the standard angle-space convention differs by :math:`\pi/m`
	in the orientation, because a ray at height ``r`` leaves at angle
	``-r/f``.

	Examples
	--------
	>>> chi = aberration_phase((64, 64), 1e-6, 1e-6, 2.5e-12,
	...                        {'C3': 1e-3, 'A1': (2e-8, 0.3)}, 22.2)  # doctest: +SKIP
	"""
	if power == 0:
		raise ValueError("aberration_phase needs a nonzero focal power: the pupil "
						 "angle theta = r/f is undefined for a lens with no power.")
	X, Y = transverse_coordinates(shape, dx, dy)
	theta = np.hypot(X, Y) * abs(power)
	phi = np.arctan2(Y, X)
	k = 2 * np.pi / wavelength
	chi = np.zeros(shape, dtype=float)
	for name, spec in (coefficients or {}).items():
		if name not in KRIVANEK_TERMS:
			raise KeyError(f"{name!r} is not a Krivanek term this function knows; "
						   f"expected one of {sorted(KRIVANEK_TERMS)}.")
		n, m = KRIVANEK_TERMS[name]
		if np.ndim(spec) == 0:
			value, angle = float(spec), 0.0
		else:
			try:
				value, angle = (float(v) for v in spec)
			except (TypeError, ValueError):
				raise ValueError(f"coefficient {name!r} must be a value or a "
								 f"(value, angle) pair, got {spec!r}.") from None
			if m == 0 and angle != 0.0:
				raise ValueError(f"{name!r} is rotationally symmetric (m = 0) and "
								 "has no orientation; drop the angle.")
		if value == 0.0:
			continue
		term = theta**(n + 1) / (n + 1) * value
		if m:
			term = term * np.cos(m * (phi - angle))
		chi -= k * term
	return chi
...
def spherical_phase(shape: tuple, dx: float, dy: float, wavelength: float,
	...
	"""
	# C3 IS spherical aberration in Krivanek notation, so delegate rather than
	# carry a second expression that could drift from it
	return aberration_phase(shape, dx, dy, wavelength, {'C3': Cs}, power)
