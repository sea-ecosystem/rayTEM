...
def I_crit_from_2D():
	...
	rundict = { "A":["CL2","CL3",0], "B":["CL2","CL3",.12345], # which CL, is used to focus to which other CL, and what's the 3rd's value
	...
	if os.path.exists("C1_vs_beamcurrent.npy"):
		...
		C1s = np.arange(41)*.012345 ; beamcurrent = []
		for C1 in tqdm(C1s):
			for f in files:
				if "C1_"+str(C1) in f:
...
def analytical_crossover_fitting():
	...
	def dz(C1,z0):
		deltas = []
		for k,(lensA,lensB,zval,Icrit) in rundict.items(): # A,(CL2,CL3,0,0.12345) --> "CL2 focuses into CL3 at 123.45mA when CL1 is zero"
			if lensA!="CL1":
				continue
	...
	microscope["CL1"].calibration_from_f_and_I(1/C1/(.23456)**2,.23456)
	...
	x=minimize(dz,x0=[1])
	C2 = x['x']
	microscope["CL2"].calibration_from_f_and_I(1/C2/(.23456)**2,.23456)
	...
	x=minimize(dz,x0=[1],args=(z5))
	C3 = x['x']
	microscope["CL3"].calibration_from_f_and_I(1/C3/(.23456)**2,.23456)
...
def visualize_fitted():
	...
	C1s,beamcurrent = np.load("C1_vs_beamcurrent.npy").T
	beamcurrent_2 = beamcurrent[C1s<.34567] ; C1s_2=C1s[C1s<.34567]
	...
	for c1 in C1s_2:
		...
		modelcurrent.append( r1.I_per_ray[-1,-1] )
...
def analytical_crossover_fitting2():
	...
	C1s,beamcurrent = np.load("C1_vs_beamcurrent.npy").T
	#beamcurrent = beamcurrent[C1s<.34567] ; C1s=C1s[C1s<.34567]
	mask = np.zeros(len(C1s)) ; mask[C1s<.31234]=1 ; mask[C1s>.45678]=1
	beamcurrent = beamcurrent[mask==1] ; C1s=C1s[mask==1]
	beamcurrent/=np.amax(beamcurrent) #beamcurrent[np.argmin(np.absolute(C1s-.34567))]
	#mask = np.zeros(len(C1s))
	#mask[beamcurrent>np.amax(beamcurrent)/2]=1		# FWHM
	#mask[beamcurrent>np.mean(beamcurrent)]=1
	#for i in range(len(mask)):						# FILL IN HOLES (if i'm below FWHM, but i have points above and below me which are, i'm probably the noise in the middle of the peak)
	...
	#guesses = (1.2345e-02, 2.3456e-01, .12345)
	...
	C1s,beamcurrent = np.load("C1_vs_beamcurrent.npy").T
	beamcurrent/=np.amax(beamcurrent[mask==1]) #beamcurrent[np.argmin(np.absolute(C1s-.34567))]
	modelcurrent = dz(C1s,*res)
	norm = beamcurrent[np.argmin(np.absolute(C1s-.34567))]
	beamcurrent/=norm ; modelcurrent/=norm
	plot([C1s,C1s,[0,.34567]],[beamcurrent,modelcurrent,[1,1]],title="fit"+str(res),markers=['','','-'],xlim=[0,.6],ylim=[0,4],filename="CLs.svg")
