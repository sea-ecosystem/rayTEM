...
def sweep_fitting_1D_simultaneous(microscope_name="U100"):
	...
	#style = "theta;OL;B*x" ; guesses = [.17,1.03,2]
	style = "theta;OL;B;B;B;B" ; guesses = [.17,1.03,np.sqrt(9.177e-02),np.sqrt(8.636e-02),np.sqrt(8.882e-02),np.sqrt(1.694e-01) ]
...
def analytical_crossover_fitting():
	...
	#for l in lines: # e.g. "PL1,-0.018678206580768512\n"
	#	k,v = l.split(",")
	#	vals = [0,0,0,0] ; vals[int(k.replace("PL",""))-1] = float(v)
	#	states.append( vals+["0/"+k.replace("PL","P")+"/CCD"] )	# -0.018678206580768512,0,0,0,0/P1/CCD --> PL1-PL4 set to...yields triangle from z0 to P1 to CCD

	lines = open("Icrit_from_2D.csv").readlines()
	for l in lines:	# e.g. "P2,P4,571.8718718718719,P1=500"
		k1,k2,v,extra = l.split(",")
		vals = [0,0,0,0] ; vals[int(k1.replace("P",""))-1] = float(v)/1000	# 0,0.5718718718718719,0,0
		if "=" in extra:
			k,v = extra.split("=")
			vals[int(k.replace("P",""))-1] = float(v)/1000	# 0.5,0.5718718718718719,0,0
	...
	microscope.adjust_length("PL1", L=7)
	microscope.adjust_length("PL2", L=11)
	microscope.adjust_length("PL3", L=11)
	microscope.adjust_length("PL4", L=6)
	...
	z["P4"]-=2 ; microscope["projector"].move("PL4",dz=-2)
	...
	#fit_vars = ["C1","C2","C3","C4"] ; guesses = [ 1.12, 1.12, 1.12, 1.55 ] ; bounds = [[0,10],[0,10],[0,10],[0,10]]
	#fit_vars = ["C1","C2","C3","C4","dz1","dz2","dz3","dz4","dzCCD"] ; guesses = [.1,.1,.1,.18,0,0,0,0,0] ; bounds = [[0,10],[0,10],[0,10],[0,10],[-10,10],[-10,10],[-10,10],[-10,10],[-10,10]]
	#fit_vars = ["C1","C2","C3","C4","dzCCD"] ; guesses = [.1,.1,.1,.18,0] ; bounds = [[0,10],[0,10],[0,10],[0,10],[-10,10]]
	#fit_vars = ["C1","C2","C3","C4","B1","B2","B3","B4"] ; guesses = [.09,.09,.09,.16,4e-3,4e-3,4e-3,4e-3] ; bounds = [[0,10],[0,10],[0,10],[0,10],[0,10],[0,10],[0,10],[0,10]]
	#fit_vars = ["C1","C2","C3","C4","dz2"] ; guesses = [ 1., 1., 1., 1.5 ,0 ] ; bounds = [[0,10],[0,10],[0,10],[0,10],[-20,20]]
	...
	#fit_vars = ["C2","C3","C4"] ; guesses = [1.12, 1.12, 2 ] ; bounds = [[0,10],[0,10],[0,10]] # use with L=0.08
	#fit_vars = ["C2","C3","C4"] ; guesses = [0.3, .3, .4 ] ; bounds = [[0,10],[0,10],[0,10]] # fits from ^^ for L=1
	#fit_vars = ["C2","C3","C4"] ; guesses = [0.14, .14, .19 ] ; bounds = [[0,10],[0,10],[0,10]] # fits from ^^ for L=5
	#fit_vars = ["C2","C3","C4"] ; guesses = [0.1, .1, .2 ] ; bounds = [[0,10],[0,10],[0,10]] # fits from ^^ for L=20,10,10,5
	#fit_vars = ["C2","C3","C4","dz1","dz2","dz3","dz4"] ; guesses = [0.1, .1, .19 ,0,0,0,0] ; bounds = [[0,10],[0,10],[0,10],[-10,10],[-10,10],[-10,10],[-10,10]]
	#fit_vars = ["C2","C3","C4"] ; guesses = [0.1, .1, .19 ] ; bounds = [[.08,.12],[.08,.12],[.15,.25]] # use with L=_,10,10,5, finds 0.09 which translates to 0.9 RPA, and we're targetting [1.16806202, 1.05487207, 1.04739167, 1.03587121]
	#fit_vars = ["C2","C3","C4"] ; guesses = [0.095, .095, .175 ] ; bounds = [[.08,.12],[.08,.12],[.15,.2]] # use with L=_,10,10,5
	# THIS YIELDS DEAD-ON CALS (once we moved P4 slightly). SET VALUES TO CALIBRATIONS SO THEY GET INHERITED ON NEXT ITERATION
	#PL234 = [1.122e+00,1.119e+00,1.512e+00] # use with L=0.08
	#PL234 = [ 0.09934745653453116, 0.09977459920479925, 0.1890652206794249 ] # use with L=20,10,10,5
	PL234 = [ 0.094738, 0.095369, 0.173185] # use with L=_,11,11,6
	...
	#fit_vars = ["C1","z0"] ; guesses = [1.12,156 ] ; bounds = [[0,10],[120,170]] # fitting for z0 is weird (still just always returning the guess), so loop z0 below and check residual
	#fit_vars = ["C1"] ; guesses = [1.12] ; bounds = [[0,10]] # use with L=0.08
	#fit_vars = ["C1","OL"] ; guesses = [.1,1.04] ; bounds = [[.05,.2],[0.95,1.1]] # use with L=20
	#fit_vars = ["C1","OL"] ; guesses = [.1,1.05] ; bounds = [[0,10],[.9,1.1]] # use with L=15
	fit_vars = ["C1"] ; guesses = [.1] ; bounds = [[.05,.2]] # okay for L=7, which finds C=.168, which gives correct rotation .16

	microscope["OL2"].calibration = 1.042 #; microscope["PL1"].strength = 0 ; microscope["PL2"].strength = 0 ; microscope["PL3"].strength = 0 ; microscope["PL4"].strength = 0 ; scope = microscope["sample":] ; scope.insert(0,Source()) ; scope.show()
	...
	def dz(vals,z0=None,printed=[False]): # vals are our calibration factors C
		...
		for state in states: # e.g. [-0.018678206580768512, 0, 0, 0, '0/P1/CCD'] or [0.5, 0, 0.5832332332332333, 0, '0/P1/P3/P4']
			if not printed[0]:
				print("state",state)
	...
	#x_PLs['x']=[1.122e+00 , 1.119e+00 , 1.49e+00]
	...
	for n in range(1,5):
		...
		C,L = microscope2["PL"+str(n)].get_C_L_from_rotation(.25,rotationPerAmp)
...
def visualize_fitted():
	...
	#microscope["PL1"].strength = .25 ; microscope["PL2"].strength = 0 ; microscope["PL3"].strength = .25 ; #microscope["PL4"].strength = 0 ; microscope.show()
	...
	for n in range(2,5):
		...
		PL1s = np.linspace(0,.75,100) ; PLZs = np.linspace(0,.75,101)
...
def lacyRotationOpenCV():
	...
	a = np.load('/mnt/U100/Users/Thomas Pfeifer/2026_04_03_U100/TWP_code/sea-pearl/TWP20260402/tools/calsweeps/frame_PL1_0.0_PL2_750.0.npy')
...
def measureLacyRotation():
	...
	if PL=="P1":
		...
		f1 = "/mnt/emmadrive/Staff/Hoglund/Pfeifer/MACSTEM/TWP20260517/tools/calsweeps_PL1450mA/"+PL+"_0.0.npy"

	ary_1,theta_1 = orthogonalize(f1)

	strengths = np.arange(1,16)*50.
	ANGLES = []
	for PZ in strengths:
		#PZ=700.0
		#f2 = '/mnt/U100/Users/Thomas Pfeifer/2026_04_03_U100/TWP_code/sea-pearl/TWP20260402/tools/calsweeps/frame_PL1_0.0_PL2_600.0.npy'
