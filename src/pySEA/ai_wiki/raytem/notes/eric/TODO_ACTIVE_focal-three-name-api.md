...
- [ ] elements.py: `focal_length` = EFL (thin path unchanged),
      new signed `back_focal_distance`, docstrings for all three.
- [ ] tests: rework `test_thick_lens_efl_vs_bfd_split` into the physics
...
- [ ] docs: interactive figure `docs/_static/thick_lens_focal_geometry.html`
      (scrubbed of discussion framing), terminology.md updated to the
      three-name scheme, wiki Lens note, site rebuilt.
- [ ] issue #11 comment proposing the adopted scheme with commit pointer.
- [ ] suite green, examples 05/06 unchanged numbers, branches in sync.
