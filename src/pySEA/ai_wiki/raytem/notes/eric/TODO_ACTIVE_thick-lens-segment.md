...
- [ ] `waveoptics.scaled_delta_tau_lens(dz, s0, R0, K)` — closed form
      `[tan(KL - phi) + tan(phi)]/(K C^2)`
- [ ] `waveoptics.propagate_thick_lens_scaled(...)` — frame (s, R) via the
...
- [ ] element seam: `Element.scaled_segment()` (None) / `Lens.scaled_segment()`
      (quadratic, K) so `L == 0` keeps the thin-lens path and `L > 0` takes
      the segment path — both kept
- [ ] tests: thick-lens crossover now matches the ray/analytic value; thin
      lens bit-for-bit unchanged; energy; dtau vs numerical integral
- [ ] wave rotation through the thick lens (Larmor -K*L), band-limited via
