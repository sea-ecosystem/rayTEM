...
- [x] Step 1 — Source seeding API renamed to match the mode names
...
- [x] Step 2 — driver kwarg `field0` → `wave0` on Section/Microscope
      `propagate_wave`/`propagate_wave_scaled`; regenerate
      `microscopes/basic_column.sea` (stored attribute names changed)
- [x] Step 3 — tests/examples/wiki/docs sync; full suite green
