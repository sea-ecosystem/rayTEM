...
- [ ] Element.zone_power_shift(h): per-axis power change ΔP(h) an aberrated
      element applies to the pupil zone at height h, from the existing
      analytic kick (deflection_at / h) — no new per-term code.
- [ ] focal_surface(method='frame'): closed-form aberrated focal surface by
...
- [ ] Covariance: Gaussian-closure aberration update in propagate_moments —
...
- [ ] Tests: closure vs Monte-Carlo ray statistics; aberrated waist shift;
      frame surface vs traced focal_surface vs closed form; idle bit-for-bit.
- [ ] Wiki/docs sync; suite green; merged to dev.
