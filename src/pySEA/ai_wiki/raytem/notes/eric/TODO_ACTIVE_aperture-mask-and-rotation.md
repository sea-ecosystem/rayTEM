...
- [ ] Rename: `Quadrapole.skew` → `rotation` (declared on `Element`, ignored
      by rotationally symmetric elements); the Larmor bookkeeping attribute
      `rotation` → `larmor_rotation` everywhere.
- [ ] `Aperture` ray path becomes a true MASK (Thomas's original option 1 in
...
- [ ] plot2D truncates dead rays (NaN beyond the plane where I hits 0).
- [ ] ex07 predict/overlay logic reworked for masking; eight-config tests
      re-verified; test_every_element fixture checked.
- [ ] Docs (wiki Aperture/Lens notes, propagation_modes/operating notes if
