...
|---------|------|
| Overview | 52 |
| Module-level functions | 81 |
|   `columnByName(name)` | 83 |
|   `fix_mat_dims(m, columnNames)` | 87 |
|   `fix_ray_dims(rays, columnNames)` | 91 |
| Element | 97 |
|   `Element.__init__(self, name, kind)` | 101 |
|   `Element.transfer_matrix(self)` | 105 |
|   `Element.propagate_moments` — analytic aberrations (2026-08-30) | 109 |
|   `Element.propagate_ray(self, r0, z, z0)` | 123 |
|   `Element.phase_shift(self, dimensions, wavelength, scaled=False, s=1.0)` | 129 |
|   `Element._scaled_segment(self)` | 193 |
|   `Element.propagate_wave(self, signal, mode='fixed', s_min=1e-3, log=None, absorb=0.1, crossover='flat', rotate=False)` | 204 |
|   `Element.propagate(self, *args, kind='ray', **kwargs)` | 240 |
| Source | 249 |
|   `Source.__init__(self, name, size, np_xy, angle, na_xy, position)` | 253 |
|   `Source.rays(self)` | 260 |
|   `Source.propagate_ray(self, r0, **kwargs)` | 264 |
|   `Source.wave(self, mode='fixed')` / `Source._aperture_wave(self, radius)` | 268 |
| Aperture | 282 |
|   `Aperture.__init__(self, name, radius, calibration, position)` | 286 |
|   `Aperture.propagate_ray(self, r0, z, z0)` | 290 |
|   `Aperture.propagate_wave(self, signal, mode='fixed', ...)` | 294 |
| Drift | 310 |
|   `Drift.__init__(self, name, length, calibration, position)` | 314 |
|   `Drift.transfer_matrix(self)` | 318 |
| Quadrapole | 324 |
|   `Quadrapole.__init__(self, name, position, length, strength, calibration)` | 328 |
|   `Quadrapole.transfer_matrix(self)` | 332 |
| AberrationScreen | 379 |
| Dipole | 399 |
|   `Dipole.__init__(self, name, position, length, strength, calibration, axis)` | 403 |
|   `Dipole.transfer_matrix(self)` | 407 |
|   `Dipole.propagate_ray(self, r0, z, z0)` | 411 |
| Lens | 417 |
|   `Lens.__init__(self, name, length, strength, calibration, position)` | 440 |
|   `Lens.transfer_matrix(self)` | 448 |
|   `Lens.calibration_from_f_and_I(self, f, I, rotationPerAmp)` | 455 |
| Prism | 461 |
|   `Prism.__init__(self, name, position, length, radius, angle, w, g, k1, strength, calibration)` | 465 |
|   `Prism.focus_matrix(self)` | 469 |
|   `Prism.bending_matrix(self, s)` | 473 |
|   `Prism.transfer_matrix(self)` | 477 |
...
### `Element.propagate_moments` — analytic aberrations (2026-08-30)

The moments mode now sees aberrations without sampling: linear kicks (C10,
aligned C12) fold into the matrix exactly, and the cubic spherical kick
(C30) enters via **Gaussian closure** — `_aberration_moment_pieces` reduces
the kick's cross- and self-moments to closed-form polynomials in Σ by
Isserlis' theorem (assumes a centered Gaussian with decoupled transverse
planes; re-Gaussianizes per element; a thick body applies its aberration as
if thin; orders above three are ignored here and carried by the ray/wave
paths). Verified against Monte-Carlo statistics of the exact per-ray kick.
...
from `deflection_at` with no per-term code.
