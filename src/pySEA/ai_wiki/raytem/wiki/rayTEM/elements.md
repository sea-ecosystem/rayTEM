...
|   `columnByName(name)` | 85 |
|   `fix_mat_dims(m, columnNames)` | 89 |
|   `fix_ray_dims(rays, columnNames)` | 93 |
| Element | 99 |
...
- `Element._aberration_monomials(P)` returns `{column: [(coef, indices), ...]}`
...
### `Element.chromatic_aberration`, `chromatic_kick`, `_chromatic_monomials` (2026-08-30)

Chromatic is **not** a Krivanek term but it lives **inside** the
`Aberrations` set, as the `'Cc'` entry (`aberrations.CHROMATIC_TERM`). Both
halves of that are deliberate:

- *In the set*, so one declaration carries everything the element does beyond
  its matrix. It serializes with the aberrations, is detached with them by
  `suspended_aberrations` (so `apply_aberrations=False` really is an ideal
  column, which it was NOT when chromatic was a separate attribute), and
  cannot be left behind when they are cleared.
- *Out of `names`/`items()`*, so the Krivanek machinery never iterates onto a
  term it cannot interpret. `deflection_at`/`phase_at` guard on
  `if not self.items()`, not `if not self`, precisely so a chromatic-only set
  takes the "nothing to evaluate" branch.

`Element.chromatic_aberration` is a property over `self.aberrations.chromatic`
with a setter that creates a set if there is none. `Aberrations.__bool__`
counts chromatic, so the gates in `propagate_moments` and
`suspended_aberrations` see a chromatic-only element as non-ideal.

The physics: `δθ = C_c P² δ x` with `δ = (E − E₀)/E₀`. That product of two
state components is bilinear, so it cannot be a transfer-matrix entry even
though it is "just a power change" — the power change differs per electron.
...
- Serialization is solved by living in the set: `Aberrations` is a
  `SEASerializable`, and `Microscope.save`'s hand-rolled JSON writer now
  encodes it through `to_metadata()` (the flat real-valued `name.a`/`name.b`
  form) with `from_metadata` on the way back. Before this an aberrated column
  could not be written to `.json` **at all** — complex coefficients and a
  nested SEA object were both unencodable.
- Chromatic needs no round pupil, so it is applied **per axis** via
  `Element._pupil_scales()`. On a quadrupole (opposite-sign powers) that makes
